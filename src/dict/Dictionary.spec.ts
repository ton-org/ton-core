...
describe("Dictionary", () => {
	it("should parse and serialize dict from example", () => {
		...
		// Pack
		let packed = beginCell().storeDictDirect(dict).endCell();
	...
	it("should parse config", () => {
		let cell = Cell.fromBoc(
			Buffer.from(
				fs.readFileSync(__dirname + "/__testdata__/config.txt", "utf-8"),
				"base64",
			),
		)[0];
		let configs = cell
		...
		let ids: number[] = [
			0, 1, 2, 4, 7, 8, 9, 10, 11, 12, 14, 15, 16, 17, 18, 20, 21, 22, 23, 24,
			25, 28, 29, 31, 32, 34, 71, 72, -999, -71,
		];
	...
	it("should parse dictionary with empty values", () => {
		let cell = Cell.fromBoc(
			Buffer.from(fs.readFileSync(__dirname + "/__testdata__/empty_value.boc")),
		)[0];
		let testDict = Dictionary.loadDirect(
			Dictionary.Keys.BigUint(256),
	...
	it("should generate merkle updates", () => {
		...
		for (let k = 1; k <= 5; k++) {
			...
			expect(exoticMerkleUpdate(update.bits, update.refs).proofHash1).toEqual(
				Buffer.from(
