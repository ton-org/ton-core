...
describe("tuple", () => {
	it("should serialize tuple with numbers", () => {
		...
		expect(
			serialized.toBoc({ idx: false, crc32: false }).toString("base64"),
		).toEqual(
	...
	it("should serialize tuple long numbers", () => {
		...
		let serialized = serializeTuple([
			{
				type: "int",
				value: BigInt("12312312312312323421"),
			},
		]);
		expect(
			serialized.toBoc({ idx: false, crc32: false }).toString("base64"),
		...
		let serialized = serializeTuple([
			{
				type: "slice",
				cell: beginCell()
					.storeCoins(BigInt("123123123123123234211234123123123"))
					.endCell(),
			},
		]);
		expect(
		...
		let serialized = serializeTuple([
			{
				...
				cell: beginCell()
					.storeAddress(
						Address.parse("kf_JuhGlmBjeEKBjLssAuQxeUc6N-CH6bKaN5fjrfhqFpqVQ"),
					)
					.endCell(),
			},
		]);
		expect(
			serialized.toBoc({ idx: false, crc32: false }).toString("base64url"),
		).toEqual(golden);
	});
	...
	it("should serialize tuples", () => {
		...
		expect(gs.toBoc({ idx: false, crc32: false }).toString("base64")).toEqual(
			golden,
		);
