...
describe("boc", () => {
	...
	it("should parse accountState.txt", () => {
		let boc = Buffer.from(
			fs.readFileSync(__dirname + "/__testdata__/accountState.txt", "utf8"),
			"base64",
		);
		let c = deserializeBoc(boc)[0];
		let b = serializeBoc(c, { idx: false, crc32: true });
	...
	it("should parse accountProof.txt", () => {
		let boc = Buffer.from(
			fs.readFileSync(__dirname + "/__testdata__/accountProof.txt", "utf8"),
			"base64",
		);
		let c = deserializeBoc(boc)[0];
		let b = serializeBoc(c, { idx: false, crc32: true });
	...
	it("should parse configProof.txt", () => {
		let boc = Buffer.from(
			fs.readFileSync(__dirname + "/__testdata__/configProof.txt", "utf8"),
			"base64",
		);
		let c = deserializeBoc(boc)[0];
		let b = serializeBoc(c, { idx: false, crc32: true });
	...
	it("should parse accountStateTest.txt", () => {
		let boc = Buffer.from(
			fs.readFileSync(__dirname + "/__testdata__/accountStateTest.txt", "utf8"),
			"base64",
		);
		let c = deserializeBoc(boc)[0];
		let b = serializeBoc(c, { idx: false, crc32: true });
	...
	it("should serialize single cell with a empty bits", () => {
		...
		expect(
			deserializeBoc(Buffer.from("te6ccgEBAQEAAgAAAA==", "base64"))[0].equals(
				cell,
			),
		).toBe(true);
		expect(
		...
		expect(
			deserializeBoc(Buffer.from("te6ccoEBAQEAAgACAAA=", "base64"))[0].equals(
				cell,
			),
		).toBe(true);
		expect(
	...
	it("should serialize single cell with a single reference", () => {
		let refCell = beginCell().storeUint(123456789, 32).endCell();
		let cell = beginCell().storeUint(987654321, 32).storeRef(refCell).endCell();
		...
		expect(
			deserializeBoc(
				Buffer.from("te6cckEBAgEADQABCDreaLEBAAgHW80VSW/75w==", "base64"),
			)[0].equals(cell),
		).toBe(true);
		expect(
			deserializeBoc(
		...
		expect(
			deserializeBoc(
				Buffer.from("te6ccsEBAgEADQAHDQEIOt5osQEACAdbzRUxP4cd", "base64"),
			)[0].equals(cell),
		).toBe(true);
	});
	...
	it("should serialize single cell with multiple references", () => {
		...
		expect(
			deserializeBoc(
				Buffer.from("te6cckEBAgEADwADCDreaLEBAQEACAdbzRWpQD2p", "base64"),
			)[0].equals(cell),
		).toBe(true);
		expect(
			deserializeBoc(
				Buffer.from("te6ccoEBAgEADwAACQMIOt5osQEBAQAIB1vNFQ==", "base64"),
			)[0].equals(cell),
		).toBe(true);
		expect(
			deserializeBoc(
				Buffer.from("te6ccsEBAgEADwAJDwMIOt5osQEBAQAIB1vNFZz9usI=", "base64"),
			)[0].equals(cell),
		).toBe(true);
	});

	it("should deserialize/serialize library cell", () => {
		let cell = Cell.fromBase64(
